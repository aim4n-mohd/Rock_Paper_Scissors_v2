...
const COLORS: Record<Faction, number> = { rock: 0x8a9a9d, paper: 0xf3e8c7, scissors: 0xe45c56 };
...
export class MeadowScene extends Phaser.Scene {
  ...
  private drawActors(): void {
    ...
    for (const unit of this.simulation.units) {
      ...
      const color = unit.flashRemainingMs > 0 ? 0xffffff : COLORS[unit.faction];
      if (unit.recruited)
    ...
    for (const particle of this.simulation.particles) {
      g.fillStyle(COLORS[particle.faction], particle.remainingMs / particle.lifetimeMs).fillRect(
